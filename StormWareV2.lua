...
local Toggle = MainTab:CreateToggle({
   ...
   Callback = function(Value)
        -- Noclip Toggle Script (Press B to toggle)
...
UserInputService.InputBegan:Connect(function(input, processed)
    if not processed and input.KeyCode == Enum.KeyCode.B then
        ...
        if noclipEnabled then
            -- Noclip mode: Allow walking through walls
            Humanoid:ChangeState(Enum.HumanoidStateType.Physics)
            game:GetService("RunService").Heartbeat:Connect(function()
                if noclipEnabled then
                    -- Disable collision for humanoid and parts while noclip is active
                    ...
                    -- Re-enable collision for humanoid and parts when noclip is off
                    for _, part in pairs(Character:GetChildren()) do
                        if part:IsA("BasePart") then
...
-- Keep the character moving with camera during noclip mode
game:GetService("RunService").Heartbeat:Connect(function()
    if noclipEnabled then
        -- Prevent falling and maintain position when noclip is enabled
